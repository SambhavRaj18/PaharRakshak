...
async function handleAiQuery() {
  ...
  if (askBtn) {
    ...
    askBtn.innerText = 'Consulting On-Device AI Engine...';
  }

  const result = await aiEngine.queryEmergencyKnowledgeBase(query, emergencyKb, sheltersData, userLocation);

  if (answerBox) {
    const badgeText = result.hasGenAi ? `🧠 Generated by: ${result.backend}` : `🤖 Grounded Local RAG (${result.backend})`;
    answerBox.innerHTML = `
